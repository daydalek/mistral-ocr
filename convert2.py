...
def process_pdf(pdf_path, api_key, progress_callback=None, output_base_dir=None):
    ...
    # 使用指定的保存路径或默认路径
    if output_base_dir:
        output_dir = os.path.join(output_base_dir, f"ocr_results_{pdf_file.stem}")
    else:
...
class OCRApp(tk.Tk if not TKDND_AVAILABLE else TkinterDnD.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Mistral OCR PDF 转换工具")
    ...
    def create_widgets(self):
        ...
        # 任务队列区域
        queue_frame = tk.LabelFrame(main_frame, text="任务队列", font=("微软雅黑", 10, "bold"), bg="#f5f5f5")
        ...
        # 输出设置区域
        output_frame = tk.LabelFrame(main_frame, text="输出设置", font=("微软雅黑", 10, "bold"), bg="#f5f5f5")
        ...
        # 进度区域
        progress_frame = tk.LabelFrame(main_frame, text="处理进度", font=("微软雅黑", 10, "bold"), bg="#f5f5f5")
    ...
    def on_drop(self, event):
        ...
        if isinstance(files, str):
            ...
            files = files.strip('{}')
            # 检查是否有多个文件（以空格分隔）
            if ' ' in files and ('"' in files or "'" in files):
                # 按照引号分割多个文件
                import re
    ...
    def prompt_for_api_key(self):
        ...
        def toggle_show():
            ...
            show_btn.config(text="隐藏" if self.show_key else "显示")
        
        show_btn = ttk.Button(entry_frame, text="显示", width=5, command=toggle_show)
        ...
        def save_key():
            ...
            if key:
                ...
                messagebox.showerror("错误", "API密钥不能为空", parent=api_dialog)
    ...
    def browse_results(self):
        ...
        if self.output_dirs:
            ...
            messagebox.showerror("错误", "未找到输出目录")
    ...
    def choose_output_dir(self):
        ...
        if selected_dir:
            ...
            self.status_label.config(text=f"已设置保存位置: {selected_dir}")
